...
public class HttpProxyServlet extends HttpServlet {
  ...
  public void init(ServletConfig servletConfig) throws ServletException {
    ...
    String allowedHostsValues = ServletConfigUtil.getInitParamValue(
        servletConfig, INIT_PARAM_ALLOWED_HOSTS);
    String validContentTypesValues = ServletConfigUtil.getInitParamValue(
        servletConfig, INIT_PARAM_ALLOWED_CONTENT_TYPES);
  ...
  protected void doHead(
    ...
    try {
      ...
      String url = RequestUtil.getParameter(request, PARAM_URL);
      ...
      if (url.startsWith("http://") || url.startsWith("https://")) {
        ...
        if (httpResponse.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
          ...
          if (!isValidContentType(contentTypesReturned[0])) {
            ...
            if (!isValidContentType(contentTypesReturned[0])) {
              response.sendError(HttpStatus.SC_UNSUPPORTED_MEDIA_TYPE, String.format(
          ...
          final String bodyAsString = IOUtils.toString(
              httpResponse.getEntity().getContent(), ENCODING).trim();
        ...
        response.sendError(HttpStatus.SC_BAD_REQUEST, "only HTTP(S) protocol supported");
      }
    } catch (Exception e1) {
      response.sendError(HttpStatus.SC_NOT_FOUND,
          "Some unexpected error occurred. Error text was: " + e1.getMessage());
    } finally {
      if (method != null) {
  ...
  protected void doGet(
    ...
    try {
      ...
      if (!isAllowedHost(host)) {
        //throw new ServletException("This proxy does not allow you to access that location.");
        returnExceptionMessage(response, HttpStatus.SC_FORBIDDEN,
            "This proxy does not allow you to access that location.");
        return;
      }
      ...
      if (url.startsWith("http://") || url.startsWith("https://")) {
        ...
        if (httpResponse.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
          ...
          String responseBody = IOUtils.toString(
              ...
              response.getCharacterEncoding()).trim();
      ...
      returnExceptionMessage(response, HttpStatus.SC_INTERNAL_SERVER_ERROR,
          "Some unexpected error occurred. Error text was: " + e2.getMessage());
    } finally {
      if (httpGet != null) {
        httpGet.releaseConnection();
      }
   ...
   * @return True if host is allowed or no restrictions for
   *         hosts (allowedHosts not defined) False in other case
   */
  ...
  private void returnExceptionMessage(
      HttpServletResponse response, int codeStatus, String message) throws IOException {
